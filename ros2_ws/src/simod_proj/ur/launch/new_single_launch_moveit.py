...
def launch_setup(context, *args, **kwargs):
    # Load urdf file
    with open("/ros2_ws/"+"left_resolved_paletta.urdf","r") as robot_description_file:
    ...
    robot_ = 'left'  #LaunchConfiguration('robot').perform(context)
    # Publish robot state
    ...
    static_tf_wheel_back_left = Node(
        package="tf2_ros",
        executable="static_transform_publisher",
        name="left_static_transform_publisher_back_left",
        output="log",
        arguments=["0.25", "0.15", "0", "0", "0", "0", "1", "left_summit_base_link", "left_summit_back_left_wheel_link"],
    ...
    static_tf_wheel_back_right = Node(
        package="tf2_ros",
        executable="static_transform_publisher",
        name="left_static_transform_publisher_back_right",
        output="log",
        arguments=["0.25", "-0.15", "0", "0", "0", "0", "1", "left_summit_base_link", "left_summit_back_right_wheel_link"],
        parameters=[{'use_sim_time': True}],
    )
    ...
    static_tf_wheel_front_left = Node(
        package="tf2_ros",
        executable="static_transform_publisher",
        name="left_static_transform_publisher_front_left",
        output="log",
        arguments=["-0.25", "0.15", "0", "0", "0", "0", "1", "left_summit_base_link", "left_summit_front_left_wheel_link"],
        parameters=[{'use_sim_time': True}],
    )
    ...
    static_tf_wheel_front_right = Node(
        package="tf2_ros",
        executable="static_transform_publisher",
        name="left_static_transform_publisher_front_right",
        output="log",
        arguments=["-0.25", "-0.15", "0", "0", "0", "0", "1", "left_summit_base_link", "left_summit_front_right_wheel_link"],
        parameters=[{'use_sim_time': True}],
    )
    ...
    static_tf_odom = Node(
        ...
        name="static_transform_publisher",
        output="log",
        arguments=['0', '0', '0', "0", "0", "0", "1", "left_summit_odom", "left_summit_base_footprint"],
        parameters=[{'use_sim_time': True}],
    )
    ...
    velocity_controller = Node(
        ...
        executable='spawner',
        arguments=["ur_left_joint_group_vel_controller", '-c', '/controller_manager'],
        output='screen',
    )
    ...
    joint_state_broadcaster_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['joint_state_broadcaster', '-c', '/controller_manager', "--controller-manager-timeout", "10"],
        output='screen',
    )

    real_joint_controllers = PathJoinSubstitution(
        [FindPackageShare("ur"), "config", "/ros2_ws/src/simod_proj/ur/config/left_controller.yaml"]
    )
    ...
    controller_manager = Node(
        ...
        parameters=[
            {'use_sim_time': True},
            get_package_share_directory('left_srm_simod_moveit_config')+"/config/ros2_controllers.yaml"],
        output='screen',
