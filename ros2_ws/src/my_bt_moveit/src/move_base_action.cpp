...
BT::NodeStatus MoveBaseAction::tick() {
    ...
    auto cmd_pub = node->create_publisher<geometry_msgs::msg::Twist>(right_cmd_vel, 10);

    // BUILD TWIST MESSAGE, ONLY RIGHT BASE FOR NOW
    geometry_msgs::msg::Twist cmd_msg;
    cmd_msg.linear.x = right_xy_vel[0];
    cmd_msg.linear.y = right_xy_vel[1];
    cmd_msg.angular.z = 0.0;

    RCLCPP_INFO(node->get_logger(), "Moving with x: %.4f, y: %.4f, theta: %.4f", right_xy_vel[0], right_xy_vel[1], 0.0);
